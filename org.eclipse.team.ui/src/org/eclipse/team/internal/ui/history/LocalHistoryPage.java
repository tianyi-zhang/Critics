...
public class LocalHistoryPage extends HistoryPage implements IHistoryCompareAdapter {
	...
	private void contributeActions() {
		...
		// Search by example
		// searchByExample = new Action(TeamUIMessages.LocalHistoryPage_GroupRevisionsByDateAction, TeamUIPlugin.getImageDescriptor(ITeamUIImages.IMG_DATES_CATEGORY)){
		// public void run() {
		// groupingOn = !groupingOn;
		// store.setValue(IPreferenceIds.PREF_GROUPBYDATE_MODE, groupingOn);
		// refreshHistory(false);
		...
		// };
		// groupingOn = store.getBoolean(IPreferenceIds.PREF_GROUPBYDATE_MODE);
		// searchByExample.setChecked(groupingOn);
		// searchByExample.setToolTipText(TeamUIMessages.LocalHistoryPage_GroupRevisionsByDateTip);
		// searchByExample.setDisabledImageDescriptor(TeamUIPlugin.getImageDescriptor(ITeamUIImages.IMG_DATES_CATEGORY));
		// searchByExample.setHoverImageDescriptor(TeamUIPlugin.getImageDescriptor(ITeamUIImages.IMG_DATES_CATEGORY));
		...
		if (!historyPageSite.isModal()) {
			...
			treeViewer.getTree().addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					...
					// mksong Oct 21, 2013 10:19:12 PM
					recordSelectedLocalFileRevision((IStructuredSelection) treeViewer.getSelection());
				}

				void recordSelectedLocalFileRevision(IStructuredSelection structuredSelection) {
					if (structuredSelection.getFirstElement() instanceof LocalFileRevision) {
						LocalFileRevision localFRev = (LocalFileRevision) structuredSelection.getFirstElement();
						try {
							IFileState fs = localFRev.getState();

							String contents = UTFile.getStringFromInputStream(fs.getContents());
							UTLocalFileHistory.setSelectedLocalFileContents(contents);
							UTLocalFileHistory.setSelectedLocalFilePath(fs.getFullPath());
							UTLocalFileHistory.setSelectedLocalFileModTime(fs.getModificationTime());
			...
			new OpenAndLinkWithEditorHelper(treeViewer) {
				protected void open(ISelection selection, boolean activate) {
					if (getSite() != null && selection instanceof IStructuredSelection) {
						...
						// mksong Oct 20, 2013 6:15:20 PM
						recordSelectedLocalFileRevision(structuredSelection);
				...
				protected void recordSelectedLocalFileRevision(IStructuredSelection structuredSelection) {
					if (structuredSelection.getFirstElement() instanceof LocalFileRevision) {
						LocalFileRevision localFRev = (LocalFileRevision) structuredSelection.getFirstElement();
						try {
							IFileState fs = localFRev.getState();
	 ...
	 * 
	 * @param parent
	 *            the parent composite to contain the group
	...
	private boolean confirmOverwrite() {
		if (file != null && file.exists()) {
			...
			IHistoryPageSite parentSite = getHistoryPageSite();
			final MessageDialog dialog = new MessageDialog(parentSite.getShell(), title, null, msg, MessageDialog.QUESTION, new String[] { IDialogConstants.YES_LABEL,
