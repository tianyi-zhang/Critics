 ...
 * then the classes to list are discovered by the <code>GenericPropertiesCreator</code> class (it checks the complete classpath).
 * 
...
public class GenericObjectEditor implements PropertyEditor, CustomPanelSupplier {
			...
			if (m_CapabilitiesFilter != null) {
				...
				if (m_Capabilities != null) {
					if (m_Capabilities.supportsMaybe(m_CapabilitiesFilter) && !m_Capabilities.supports(m_CapabilitiesFilter))
						result = Messages.getInstance().getString("GenericObjectEditor_GOETreeNode_ToString_Result_Text_First")
								+ MAYBE_SUPPORT
								+ Messages.getInstance().getString("GenericObjectEditor_GOETreeNode_ToString_Result_Text_Second")
								+ result
								+ Messages.getInstance().getString("GenericObjectEditor_GOETreeNode_ToString_Result_Text_Third");
					else if (!m_Capabilities.supports(m_CapabilitiesFilter))
						result = Messages.getInstance().getString("GenericObjectEditor_GOETreeNode_ToString_Result_Text_Fourth")
								+ NO_SUPPORT
								+ Messages.getInstance().getString("GenericObjectEditor_GOETreeNode_ToString_Result_Text_Fifth")
								+ result
								+ Messages.getInstance().getString("GenericObjectEditor_GOETreeNode_ToString_Result_Text_Sixth");
				}
			}
	...
	public class CapabilitiesFilterDialog extends JDialog {
		...
		/** the OK button. */
		protected JButton		m_OkButton			= new JButton(Messages.getInstance().getString(
		...
		protected void initGUI() {
			...
			m_InfoLabel.setText(Messages.getInstance().getString("GenericObjectEditor_InitGUI_InfoLabel_SetTitle_Text_First")
	...
	public class JTreePopupMenu extends JPopupMenu {
		...
		private JButton		m_FilterButton			= new JButton(Messages.getInstance().getString(
															"GenericObjectEditor_JTreePopupMenu_FilterButton_JButton_Text"));

		/** The remove filter button in case of CapabilitiesHandlers. */
		private JButton		m_RemoveFilterButton	= new JButton(Messages.getInstance().getString(
															"GenericObjectEditor_JTreePopupMenu_RemoveFilterButton_JButton_Text"));

		/** The button for closing the popup again. */
		private JButton		m_CloseButton			= new JButton(Messages.getInstance().getString(
															"GenericObjectEditor_JTreePopupMenu_CloseButton_JButton_Text"));
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				try {
					...
					if (!m_ClassType.isAssignableFrom(obj.getClass())) {
						throw new Exception(Messages.getInstance().getString("GenericObjectEditor_OpenObject_Exception_Text")
								+ m_ClassType.getName());
					}
					return obj;
				} catch (Exception ex) {
	...
	public static void registerEditors() {
		...
		// mksong Nov 28, 2013 11:39:00 PM
		boolean useGuiEditors = Boolean.valueOf(props.getProperty("USEGUIEDITORS"));
		if (useGuiEditors)
		...
		while (enm.hasMoreElements()) {
			...
			try {
				...
				System.err.println(Messages.getInstance().getString(
						...
						+ name
						+ Messages.getInstance().getString("GenericObjectEditor_RegisterEditors_Exception_Error_Text_Second")
						+ value
						+ Messages.getInstance().getString("GenericObjectEditor_RegisterEditors_Exception_Error_Text_Third") + e);
			}
		}
	}
	...
	public void paintValue(java.awt.Graphics gfx, java.awt.Rectangle box) {
		...
		if (m_Enabled) {
			...
			*/
			...
			if (m_Object instanceof OptionHandler) {
				gfx.drawString(" " + Utils.joinOptions(((OptionHandler) m_Object).getOptions()), repwidth + 2, fm.getAscent()
						+ vpad);
			}
		}
	}
