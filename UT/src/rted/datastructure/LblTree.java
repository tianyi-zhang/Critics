...
public class LblTree extends DefaultMutableTreeNode implements Comparable {
	...
	public void prettyLog(boolean printTmpData) {
		for (int i = 0; i < getLevel(); i++) {
			UTLog.print(false, TAB_STRING);
		}
		if (!isRoot()) {
			UTLog.print(false, BRANCH_STRING);
		} else {
			if (getTreeID() != NO_TREE_ID) {
				UTLog.println(false, "treeID: " + getTreeID());
			}
			UTLog.print(false, ROOT_STRING);
		}
		UTLog.print(false, " '" + this.getLabel() + "' ");
		if (printTmpData) {
			UTLog.println(false, "" + getTmpData());
		} else {
			UTLog.println(false, "");
		}
		for (Enumeration e = children(); e.hasMoreElements();) {
			((LblTree) e.nextElement()).prettyLog(printTmpData);
		}
	}
	 ...
	 * 
	 * @param s
	 *            string representation of a tree. Format: "treeID:{root{...}}".
	 * @return tree represented by s
	 */
	public static LblTree fromString(String s) {
		int treeID = UTFormat.getTreeID(s);
		s = s.substring(s.indexOf(OPEN_BRACKET), s.lastIndexOf(CLOSE_BRACKET) + 1);
		LblTree node = new LblTree(UTFormat.getRoot(s), treeID);
		Vector c = UTFormat.getChildren(s);
