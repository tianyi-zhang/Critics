...
import rted.datastructure.LabelDictionary;
...
public class RTEDInfoTreeOpt {
	...
	// trees
	private RTEDInfoTree		it1;
	private RTEDInfoTree		it2;
	private int					size1;
	private int					size2;
	...
	private byte[][]			deltaBit;							// stores the distances difference of a form
																	// delta(F,G)-delta() for every pair of
	 ...
	 * @param matchCost
	 */
	public RTEDInfoTreeOpt(double delCost, double insCost, double matchCost) {
	...
	public void init(LblTree t1, LblTree t2) {
		...
		it1 = new RTEDInfoTree(t1, ld);
		it2 = new RTEDInfoTree(t2, ld);
		size1 = it1.getSize();
		size2 = it2.getSize();
		...
		for (int x = 0; x < sizes1.length; x++) { // for all nodes of initially left tree
			for (int y = 0; y < sizes2.length; y++) { // for all nodes of initially right tree
				// This is an attempt for distances of single-node subtree and anything else
	 ...
	 * @return
	 */
	private double computeDistUsingStrArray(RTEDInfoTree it1, RTEDInfoTree it2) {
		...
		case HEAVY:
			...
			strStat[3]++;
			strStat[HEAVY]++;
			return spfH(it1, it2, RTEDInfoTree.toIntArray(heavyPath));
		case REVLEFT:
		...
		case REVHEAVY:
			...
			strStat[3]++;
			strStat[HEAVY]++;
			return spfH(it2, it1, RTEDInfoTree.toIntArray(heavyPath));
		default:
			return -1;
		}
	}
	 ...
	 */
	private double spfL(RTEDInfoTree it1, RTEDInfoTree it2) {
	...
	private void treeEditDist(RTEDInfoTree it1, RTEDInfoTree it2, int i, int j) {
	 ...
	 * @return distance between subtrees it1 and it2
	 */
	private double spfR(RTEDInfoTree it1, RTEDInfoTree it2) {
		...
		return it1.isSwitched() ? delta[it2.getCurrentNode()][it1.getCurrentNode()]
				+ deltaBit[it2.getCurrentNode()][it1.getCurrentNode()] * costMatch : delta[it1.getCurrentNode()][it2
				.getCurrentNode()] + deltaBit[it1.getCurrentNode()][it2.getCurrentNode()] * costMatch;
	}

	private void treeEditDistRev(RTEDInfoTree it1, RTEDInfoTree it2, int i, int j) {
		...
		for (int i1 = 1; i1 <= i - ioff; i1++) {
			for (int j1 = 1; j1 <= j - joff; j1++) {
				...
				if ((it1.info[RPOST2_RLD][i1 + ioff] == it1.info[RPOST2_RLD][i])
					...
					setDeltaValue(it1.info[RPOST2_POST][i1 + ioff], it2.info[RPOST2_POST][j1 + joff], forestdist[i1 - 1][j1 - 1],
							switched);
	 ...
	 * @return distance between subtrees it1 and it2
	 */
	private double spfH(RTEDInfoTree it1, RTEDInfoTree it2, int[] heavyPath) {
	 ...
	 * @param aStrategy
	 */
	private void computePeriod(RTEDInfoTree it1, int aVp, int aNextVp, RTEDInfoTree it2, int aStrategy) {
		...
		for (int i = gSize - 1; i >= 0; i--) {
			...
			for (int kPrime = 1; kPrime <= k; kPrime++) {
				...
				fForestPreorderKPrime = aStrategy == LEFT ? vpPreorder + (k - kPrime) : it1.info[POST2_PRE][fTreeSize - 1
				...
				for (int j = jOfI; j >= 0; j--) {
					...
					// fill S table
					s[kPrime - 1][j] = (deleteFromLeft < deleteFromRight) ? ((deleteFromLeft < match) ? deleteFromLeft : match)
			...
			if (i > 0) {
				...
				if (i + jOfIminus1 < gSize) {
					...
					gijOfIMinus1Preorder = aStrategy == LEFT ? it2.info[POST2_PRE][gTreeSize - 1 - (gRevPreorder + (i - 1))]
							: gPreorder + (i - 1);
					...
					if (k - 1 - 1 < 0) {
						if (aStrategy != previousStrategy) {
							...
							setDeltaValue(it1.info[PRE2_POST][vpPreorder], it2.info[PRE2_POST][gijOfIMinus1Preorder],
									tCOPY[previousI][i + jOfIminus1 - previousI], switched);
						} else {
							setDeltaValue(it1.info[PRE2_POST][vpPreorder], it2.info[PRE2_POST][gijOfIMinus1Preorder],
	 ...
	 * @param treeSize
	 */
	private void computeIJTable(RTEDInfoTree it, int subtreePreorder, int subtreeRevPreorder, int subtreeSize, int aStrategy,
			int treeSize) {
	 ...
	 */
	private int jOfI(RTEDInfoTree it, int aI, int aSubtreeWeight, int aSubtreeRevPre, int aSubtreePre, int aStrategy, int treeSize) {
	...
	private void forestDist(RTEDInfoTree ted1, RTEDInfoTree ted2, int i, int j, double[][] treedist, double[][] forestdist) {
		...
		for (int di = ted1.getInfo(POST2_LLD, i - 1) + 1; di <= i; di++) {
			forestdist[di][ted2.getInfo(POST2_LLD, j - 1) + 1 - 1] = forestdist[di - 1][ted2.getInfo(POST2_LLD, j - 1) + 1 - 1]
					+ costDel;
			for (int dj = ted2.getInfo(POST2_LLD, j - 1) + 1; dj <= j; dj++) {
				forestdist[ted1.getInfo(POST2_LLD, i - 1) + 1 - 1][dj] = forestdist[ted1.getInfo(POST2_LLD, i - 1) + 1 - 1][dj - 1]
						+ costIns;
				...
				if ((ted1.getInfo(POST2_LLD, di - 1) == ted1.getInfo(POST2_LLD, i - 1))
					...
					forestdist[di][dj] = Math.min(Math.min(forestdist[di - 1][dj] + costDel, forestdist[di][dj - 1] + costIns),
