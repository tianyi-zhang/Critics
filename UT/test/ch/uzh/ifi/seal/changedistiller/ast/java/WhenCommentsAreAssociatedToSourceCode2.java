...
public class WhenCommentsAreAssociatedToSourceCode2 extends JavaDistillerTestCase {
	...
	private UTChangeDistiller		changeDistiller;
	...
	@Test
	public void myTest() {
		Node ndRootOldRev = new Node(JavaEntityType.METHOD, "bar");
		ndRootOldRev.setEntity(new SourceCodeEntity("bar", JavaEntityType.METHOD, new SourceRange()));
		Node ndBlockOldRev = new Node(JavaEntityType.BLOCK, "{boolean check = (number > 0);}");
		ndRootOldRev.add(ndBlockOldRev);
		ndRootOldRev.print();
		System.out.println("------------------------------------------");

		Node ndRootNewRev = new Node(JavaEntityType.METHOD, "bar");
		ndRootNewRev.setEntity(new SourceCodeEntity("bar", JavaEntityType.METHOD, new SourceRange()));
		Node ndBlockNewRev1 = new Node(JavaEntityType.BLOCK, "{boolean check = (number > 0);}");
		Node ndBlockNewRev2 = new Node(JavaEntityType.BLOCK, "{String str = null;}");
		ndRootNewRev.add(ndBlockNewRev1);
		ndRootNewRev.add(ndBlockNewRev2);
		ndRootNewRev.print();
		System.out.println("------------------------------------------");

		changeDistiller = new UTChangeDistiller();
		changes = changeDistiller.diffBlock(ndRootOldRev, ndRootNewRev);
	...
	public void undecidedProximityRatingShouldAssociateCommentToNextEntity() throws Exception {
		...
		assertCorrectAssociation(node, "// check the interesting number" + LF + "        // and some new else", JavaEntityType.LINE_COMMENT);
	}

	public void commentInsideBlockShouldBeAssociatedInside() throws Exception {
		Node node = findNode("a = (23 + Integer.parseInt(\"42\"));");
		assertCorrectAssociation(node, "/* A block comment" + LF + "             * with stars" + LF + "             */", JavaEntityType.BLOCK_COMMENT);
