...
public class JavaDeclarationConverter extends ASTVisitor {
	...
	public void initialize(Node root, Scanner scanner, List<UTASTBindingManagerAbstract> bindingManagerList) {
		fScanner = scanner;
		fSource = String.valueOf(scanner.source);
		fNodeStack.clear();
		fNodeStack.push(root);
	...
	public boolean visit(Argument node, BlockScope scope) {
		...
		fHelper.setNewVaule(node, scope);
	...
	private void visitExpression(Expression expression) {
		if (expression != null) {
			push(fASTHelper.convertNode(expression), expression.toString(), expression.sourceStart(), expression.sourceEnd());
			pop();
		}
	}
			...
			try {
				...
				while ((token = fScanner.getNextToken()) != TerminalTokens.TokenNameEOF) {
					...
					case TerminalTokens.TokenNametransient:
						push(JavaEntityType.MODIFIER, fScanner.getCurrentTokenString(), fScanner.getCurrentTokenStartPosition(), fScanner.getCurrentTokenEndPosition());
						pop();
						break;
					default:
						break;
	...
	public boolean visit(ParameterizedSingleTypeReference type, BlockScope scope) {
		...
		String value = prefixWithNameOfParrentIfInMethodDeclaration();
		if (!value.isEmpty()) {
			String newValue = getCurrentParent().getValueNew();
			if (newValue != null) {
				fHelper.fNewNameBuf = new StringBuffer(newValue + COLON_SPACE + getSource(start, end)); // INIT & GET
				JavaNewNamePrint.printWithNewName = false; // RESET
			}
		}
		pushValuedNode(type, value + getSource(start, end));
	...
	public boolean visit(ArrayTypeReference arrayType, BlockScope scope) {
		...
		if (!value.isEmpty()) {
			String newValue = getCurrentParent().getValueNew();
			if (newValue != null) {
				fHelper.fNewNameBuf = new StringBuffer(newValue + COLON_SPACE + String.valueOf(arrayType.token)); // INIT & GET
				JavaNewNamePrint.printWithNewName = false; // RESET
			}
		}
		pushValuedNode(arrayType, value + String.valueOf(arrayType.token));
		return false;
	}
	...
	public boolean visit(TypeParameter typeParameter, BlockScope scope) {
		push(fASTHelper.convertNode(typeParameter), getSource(typeParameter.sourceStart(), typeParameter.declarationSourceEnd), typeParameter.sourceStart(), typeParameter.declarationSourceEnd);
		return false;
	}
	...
	private void visitAbstractVariableDeclarations(JavaEntityType parentLabel, AbstractVariableDeclaration[] declarations) {
		int start = -1;
		int end = -1;
