...
public class JavaMethodBodyConverter extends ASTVisitor {
	...
	private List<UTASTBindingManagerAbstract>	fBindingManagerList;
	private JavaMethodBodyConverterHelper		fHelper	= new JavaMethodBodyConverterHelper();
	...
	public void initialize(Node root, ASTNode methodRoot, List<Comment> comments, Scanner scanner, List<UTASTBindingManagerAbstract> bindingManagerList) {
		fNodeStack.clear();
		fLastAssociationCandidate.clear();
		fLastCommentNodeTuples.clear();
		fLastVisitedNode = methodRoot;
		...
		fScanner = scanner;
		fSource = String.valueOf(scanner.getSource());
		fBindingManagerList = bindingManagerList;
		fHelper = new JavaMethodBodyConverterHelper(fBindingManagerList);
	}
	...
	private void insertCommentIntoTree(Comment comment) {
		...
		push(label, getSource(comment.sourceStart(), comment.sourceEnd() - 1), comment.sourceStart(), comment.sourceEnd());
		pop(comment);
	}
	...
	private boolean isCommentBetweenCurrentNodeAndLastNode(Comment comment, ASTNode currentNode) {
		return (fLastVisitedNode.sourceStart() < comment.sourceStart()) && (comment.sourceStart() < currentNode.sourceStart());
	}
	...
	public boolean visit(Assignment assignment, BlockScope scope) {
		fHelper.setNewValue(assignment, scope);
	...
	public boolean visit(CompoundAssignment compoundAssignment, BlockScope scope) {
		fHelper.setNewValue(compoundAssignment, scope);
	...
	public boolean visit(PostfixExpression postfixExpression, BlockScope scope) {
		fHelper.setNewValue(postfixExpression, scope);
	...
	public boolean visit(PrefixExpression prefixExpression, BlockScope scope) {
		fHelper.setNewValue(prefixExpression, scope);
	...
	public boolean visit(AssertStatement assertStatement, BlockScope scope) {
		...
		push(fASTHelper.convertNode(assertStatement), value, assertStatement.sourceStart(), assertStatement.sourceEnd() + 1);
		return false;
	}
	...
	public boolean visit(DoStatement doStatement, BlockScope scope) {
		...
		fHelper.setNewValue(doStatement, scope);
	...
	public boolean visit(ForeachStatement foreachStatement, BlockScope scope) {
		...
		fHelper.setNewValue(foreachStatement, scope);
		String value = "";
		value += foreachStatement.elementVariable.printAsExpression(0, new StringBuffer()).toString();
		value += COLON;
		value += foreachStatement.collection.toString();
		pushValuedNode(foreachStatement, value);
	...
	public boolean visitExpression(Expression expression, BlockScope scope) {
		...
		if (JavaNewNamePrint.printWithNewName) {
			fHelper.fNewNameBuf = new StringBuffer(30); // INIT
			fHelper.fNewNameBuf = expression.printStatement(0, fHelper.fNewNameBuf); // GET
			JavaNewNamePrint.printWithNewName = false; // RESET
		}
		push(fASTHelper.convertNode(expression), expression.toString() + ';', expression.sourceStart(), expression.sourceEnd() + 1);
		return false;
	}
	...
	public boolean visit(ForStatement forStatement, BlockScope scope) {
		...
		if (forStatement.condition != null) {
			if (fHelper.setNewValue(forStatement, scope)) { // SET
				JavaNewNamePrint.printWithNewName = false; // RESET
			}
			value = forStatement.condition.toString(); // GET
		}
		...
		if (forStatement.initializations != null && forStatement.initializations.length > 0) {
			for (Statement initStatement : forStatement.initializations) {
				if (fHelper.setNewValue(initStatement, scope)) { // SET
					JavaNewNamePrint.printWithNewName = false;
				}
				push(JavaEntityType.FOR_INIT, initStatement.toString(), initStatement.sourceStart(), initStatement.sourceEnd());
		...
		if (forStatement.increments != null && forStatement.increments.length > 0) {
			for (Statement incrementStatement : forStatement.increments) {
				if (fHelper.setNewIncrements(incrementStatement, scope)) { // SET
					JavaNewNamePrint.printWithNewName = false;
				}
				value = incrementStatement.toString();
				push(JavaEntityType.FOR_INCR, value, incrementStatement.sourceStart(), incrementStatement.sourceEnd());
	...
	public boolean visit(IfStatement ifStatement, BlockScope scope) {
		...
		StringBuffer tmpNewName = null;
		boolean result = fHelper.setNewValue(ifStatement, scope); // SET
		if (result) {
			tmpNewName = new StringBuffer(fHelper.fNewNameBuf);
		}
		...
		if (ifStatement.thenStatement != null) {
			if (result) {
				fHelper.fNewNameBuf = new StringBuffer(30); // INIT
				fHelper.fNewNameBuf = tmpNewName; // GET
				JavaNewNamePrint.printWithNewName = false; // RESET
			}
	...
	public boolean visit(LocalDeclaration localDeclaration, BlockScope scope) {
		...
		fHelper.setNewValueLocalDeclaration(localDeclaration, scope);
	...
	public boolean visit(MessageSend messageSend, BlockScope scope) {
		...
		fHelper.setNewValue(messageSend, scope);
	...
	public boolean visit(ReturnStatement returnStatement, BlockScope scope) {
		...
		fHelper.setNewValue(returnStatement, scope);
		pushValuedNode(returnStatement, returnStatement.expression != null ? returnStatement.expression.toString() + ';' : "");
		return false;
	}
	...
	private void visitCatchClauses(TryStatement node, BlockScope scope) {
		if ((node.catchBlocks != null) && (node.catchBlocks.length > 0)) {
			...
			for (int i = 0; i < node.catchArguments.length; i++) {
				...
				push(JavaEntityType.CATCH_CLAUSE, node.catchArguments[i].type.toString(), catchClauseSourceStart, node.catchBlocks[i].sourceEnd);
	...
	public boolean visit(WhileStatement whileStatement, BlockScope scope) {
		...
		fHelper.setNewValue(whileStatement, scope);
		push(fASTHelper.convertNode(whileStatement), whileStatement.condition.toString(), whileStatement.sourceStart(), whileStatement.sourceEnd);
	...
	private void push(EntityType label, String value, int start, int end) {
		...
		if (fHelper.fNewNameBuf != null) {
			n.setNewValue(fHelper.fNewNameBuf.toString());
			fHelper.fNewNameBuf = null;
		}
		getCurrentParent().add(n);
		// /////////////////////////////////////////////////
		Node parent = getCurrentParent();
		if (parent.getEntity().getType() == JavaEntityType.FOR_STATEMENT) {
			NodeForStmt forStmt = parent.getForStmt();
			if (n.getEntity().getType() == JavaEntityType.FOR_INIT) {
				forStmt.forInit.add(n);
			}
			if (n.getEntity().getType() == JavaEntityType.FOR_INCR) {
				forStmt.forIncr.add(n);
			}
		}
		fNodeStack.push(n);
	}
