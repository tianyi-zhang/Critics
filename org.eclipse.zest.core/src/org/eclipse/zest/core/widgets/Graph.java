...
public class Graph extends FigureCanvas implements IContainer {
		...
		IFigure figureUnderMouse = this.getContents().findFigureAt(x, y, new TreeSearch() {
			...
			public boolean prune(IFigure figure) {
				...
				if (item != null && item.getItemType() == GraphItem.CONTAINER) {
					return false;
				} else if (figure instanceof FreeformLayer || parent instanceof FreeformLayer || figure instanceof ScrollPane || parent instanceof ScrollPane
						|| parent instanceof ScalableFreeformLayeredPane || figure instanceof ScalableFreeformLayeredPane || figure instanceof FreeformViewport
	...
	private void applyLayoutInternal() {
		...
		// System.out.println("[DBG] " + this.getViewport().getSize().height);
		// System.out.println("[DBG] " + this);
		// d.width = this.zestRootLayer.getBounds().width;
		// d.height = this.zestRootLayer.getBounds().height;
