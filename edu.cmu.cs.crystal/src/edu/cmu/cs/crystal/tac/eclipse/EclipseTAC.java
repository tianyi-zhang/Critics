...
//	}
	...
	public ThisVariable implicitThisVariable(IBinding accessedElement) {
		...
		try{
			...
			result = thisVar.get(thisBinding);
			if(result == null) {
	 ...
	 * @return the variable representing the given bindings
	 */
	private Variable getVariable(IBinding binding){
		...
		if(result == null) {
			if(binding instanceof IVariableBinding) {
				...
				if(method == null){
					result = new SourceVariable(vb.getName(), vb, declaredIn == null);
				}else{
	...
	private class NewInstructionVisitor extends ASTVisitor {
		...
		public boolean visit(VariableDeclarationFragment node) {
			try{
				TACInstruction result = factory.create(node, EclipseTAC.this);
				setResult(result);
			}catch(Exception e){
				noResult();
			}
