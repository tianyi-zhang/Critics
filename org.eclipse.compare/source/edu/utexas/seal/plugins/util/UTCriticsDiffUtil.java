 ...
 */
public class UTCriticsDiffUtil {
	...
	public List<NodeValueArg> getNodeValueArg(String aValue, String aValueWithParms) {
		...
		List<NodeValueArg> lstNodeValueArg = getListDiffBlock(aValue, aValueWithParms);

		for (int i = 0; i < lstNodeValueArg.size(); i++) {
			NodeValueArg elem = lstNodeValueArg.get(i);

			if (elem.toString().contains("[17, 32] (classFileReader <=> $^^)"))
				System.out.println("[DBG7] " + i + " : " + elem);
			if (i > lstVarNew.size() - 1) {
				break;
			}
			String iNewVar = lstVarNew.get(i);
			elem.setArgNew(iNewVar);
		}
		return lstNodeValueArg;
	}
	 ...
	 * @return
	 */
	public List<NodeValueArg> getListDiffBlock(String str1, String str2) {
		boolean trace = false;
		List<Integer> lstIndex = new ArrayList<Integer>();
		LinkedList<Diff> diffs = diff_main(str1, str2);
		int sumlen = 0;
		NodeValueArg nodeValueArg = null;
		List<NodeValueArg> lstNodeValueArg = new ArrayList<NodeValueArg>();
		for (Diff diff : diffs) {
			if (trace) {
				System.out.println("[DBG0] " + diff);
			}
			switch (diff.operation) {
			case DELETE:
				lstIndex.add(sumlen);
				nodeValueArg = new NodeValueArg(sumlen, sumlen + diff.text.length(), diff.text);
				lstNodeValueArg.add(nodeValueArg);
			case EQUAL:
				sumlen += diff.text.length();
				break;
			case INSERT:
				nodeValueArg.setArgNew(diff.text);
				break;
			default:
				break;
			}
		...
		if (trace) {
			System.out.println("[DBG] SUM LEN: " + sumlen);
			System.out.println("[DBG] ORG LEN: " + str1.length());
			System.out.println("------------------------------------------");
			for (int i = 0; i < lstIndex.size(); i++) {
				Integer elem = lstIndex.get(i);
