 ...
 * <LI>saving the model.
 * </UL>
 * The Compare plug-in's <code>openCompareEditor</code> method takes an <code>CompareEditorInput</code> and starts sequencing through the above steps. If the compare result is not empty a new compare editor is opened and takes over the sequence until eventually closed.
 * <p>
 * The <code>prepareInput</code> method should contain the code of the compare operation. It is executed under control of a progress monitor and can be canceled. If the result of the compare is not empty, that is if there are differences that needs to be presented, the <code>ICompareInput</code> should hold onto them and return them with the <code>getCompareResult</code> method. If the value returned from <code>getCompareResult</code> is not <code>null</code> a compare editor is opened on the <code>ICompareInput</code> with title and title image initialized by the corresponding methods of the <code>ICompareInput</code>.
 * <p>
 * Creation of the editor's SWT controls is delegated to the <code>createContents</code> method. Here the SWT controls must be created and initialized with the result of the compare operation.
 * <p>
 * If merging is allowed, the modification state of the compared constituents must be tracked and the dirty state returned from method <code>isSaveNeeded</code>. The value <code>true</code> triggers a subsequent call to <code>save</code> where the modified resources can be saved.
 * <p>
 ...
 * <p>
 * Subclasses provide custom setups, e.g. for a Catch-up/Release operation by passing a subclass of <code>CompareConfiguration</code> and by implementing the <code>prepareInput</code> method. If a subclass cannot use the <code>DiffTreeViewer</code> which is installed by default in the top left pane, method <code>createDiffViewer</code> can be overridden.
 * <p>
 * If subclasses of this class implement {@link ISaveablesSource}, the compare editor will pass these models through to the workbench. The editor will still show the dirty indicator if one of these underlying models is dirty. It is the responsibility of subclasses that implement this interface to call {@link #setDirty(boolean)} when the dirty state of any of the models managed by the subclass change dirty state.
 * 
 * @see CompareUI
 * @see CompareEditorInput
 */
public abstract class CompareEditorInput extends PlatformObject implements IEditorInput, IPropertyChangeNotifier,
		IRunnableWithProgress, ICompareContainer {
	...
	public synchronized ICompareNavigator getNavigator() {
		if (fNavigator == null)
			fNavigator = new CompareEditorInputNavigator(new Object[] { fStructureInputPane, fStructurePane1, fStructurePane2,
					fContentInputPane });
		return fNavigator;
	}
	 ...
	 *                if the <code>prepareInput</code> method must propagate a checked exception,
	...
	/**
	 * Returns the compare result computed by the most recent call to the <code>run</code> method. Returns <code>null</code> if no
	 * differences were found.
	 * 
	 * @return the compare result prepared in method <code>prepareInput</code> or <code>null</code> if there were no differences
	 */
	 ...
	 * @param cei
	 *            the compare editor input for the viewer
	 ...
	 * been created. If this method gets invoked then {@link #handleDispose()} will be invoked when the control is disposed. Subclasses may extend this
	 ...
	 *            the parent control under which the control must be created
	 * @param direction
	 *            the layout direction of the contents, either </code>SWT.HORIZONTAL<code> or </code>SWT.VERTICAL<code>
		...
		if (fStructureInputPane != null && (fInput instanceof ICompareInput || isCustomStructureInputPane())) {
			if (hasChildren(fInput) || isCustomStructureInputPane()) {
				...
				if (hasUnusableContentViewer()
						|| (structureCompareOnSingleClick() && isShowStructureInOutlineView() && !hasOutlineViewer(fInput))) {
					fStructureInputPane.setInput(fInput);
				}
			} else {
				fStructureInputPane.setInput(fInput);
	...
	private void feedDefault1(final ISelection selection) {
		BusyIndicator.showWhile(fComposite.getDisplay(), new Runnable() {
			// mksong Nov 2, 2013 7:37:00 PM
			String getPath(DiffNode diffNode, boolean isLeft) {
				String result = "";
				ITypedElement el = null;
				if (isLeft)
					el = diffNode.getLeft();
				else
					el = diffNode.getRight();
				if (el instanceof SummaryEditionNode) {
					SummaryEditionNode newRNode = (SummaryEditionNode) el;
					result = "" + newRNode.getRemoteResource();
				}
				return result;
			}

			// mksong Nov 2, 2013 7:59:59 PM
			String getFullyQualifedName(List<String> path) {
				String root = path.get(0);
				String revisionNum = root.substring(root.lastIndexOf(":") + 1);
				root = root.replace(":" + revisionNum, "");
				String elLast = path.get(path.size() - 1);
				String subroot = elLast.replace(root, "").replace(":" + revisionNum, "");
				subroot = subroot.substring(1, subroot.length()).replace("/", ".");
				return subroot;
			}
			...
			public void run() {
				if (!selection.isEmpty()) {
					...
					// troy 
					UTCriticsStructureSelection.currentSel = selection;
					// mksong Oct 24, 2013 2:23:01 PM
					final String S = System.getProperty("file.separator");
					String pathStr = "";
					List<String> parents = new ArrayList<String>();
					List<String> leftRemotePaths = new ArrayList<String>();
					List<String> rightRemotePaths = new ArrayList<String>();
					...
					if (selection instanceof TreeSelection) {
						...
						for (int i = 0; i < segmentSZ; i++) {
							...
							if (segment instanceof DiffNode) {
								...
								String result = getPath(diffNode, true);
								if (!result.isEmpty())
									leftRemotePaths.add(result);
								result = getPath(diffNode, false);
								if (!result.isEmpty())
									rightRemotePaths.add(result);
								// // String strEl = el.toString();
								// // System.out.println("[DBG] " + strEl);
								//
								// if (el instanceof SummaryEditionNode) {
								// SummaryEditionNode newRNode = (SummaryEditionNode) el;
								// ISVNRemoteResource newResource = newRNode.getRemoteResource();
								// // System.out.println("[DBG] new resource: " + newResource);
								// leftRemotePaths.add(newResource + "");
								// }
						...
						if (parents.size() == 2) {
							UTCriticsPairFileInfo.setRightFilePath(parents.get(0).trim() + S + pathStr);
							UTCriticsPairFileInfo.setLeftFilePath(parents.get(1).trim() + S + pathStr);
						}
						if (!leftRemotePaths.isEmpty()) {
							System.out.println("DBG__________________________________________");
							System.out.println("[DBG] FQ-NAME: " + getFullyQualifedName(leftRemotePaths));
						}
						if (!rightRemotePaths.isEmpty()) {
							System.out.println("[DBG] FQ-NAME: " + getFullyQualifedName(rightRemotePaths));
							System.out.println("DBG__________________________________________");
						}
					}
					// System.out.println("[DBG] " + getElement(selection));
					// ICompareInput input = (ICompareInput) getElement(selection);
	 ...
	 * is a <code>IStructuredSelection</code> with exactly one element. Returns <code>null</code> otherwise.
	 * 
	 * @param selection
	 *            the selection
	 ...
	 * can be returned. Otherwise a new viewer must be created under the given parent composite or <code>null</code> can be returned to indicate that no viewer could be found.
	 * <p>
	 ...
	 * 
	 * @param oldViewer
	 *            a new viewer is only created if this old viewer cannot show the given input
	 * @param input
	...
	public Viewer findStructureViewer(Viewer oldViewer, ICompareInput input, Composite parent) {
		return fStructureViewerDescriptor != null ? fStructureViewerDescriptor.createViewer(oldViewer, parent,
				fCompareConfiguration) : CompareUI.findStructureViewer(oldViewer, input, parent, fCompareConfiguration);
	}
	...
	public Viewer findContentViewer(Viewer oldViewer, ICompareInput input, Composite parent) {
		...
		Viewer newViewer = fContentViewerDescriptor != null ? fContentViewerDescriptor.createViewer(oldViewer, parent,
				fCompareConfiguration) : CompareUI.findContentViewer(oldViewer, input, parent, fCompareConfiguration);
	 ...
	 * right side. The value returned is the value of the <code>DIRTY_STATE</code> property of this input object.
	 * 
	 * Returns <code>true</code> if left or right side has unsaved changes
	 * Subclasses don't have to override if the functionality provided by <code>setDirty</code> is sufficient.
	 * 
	 ...
	 * By default, this method calls {@link #isSaveNeeded()} but subclasses may extend.
	 * 
	...
	/**
	 * Sets the dirty state of this input to the given value and sends out a <code>PropertyChangeEvent</code> if the new value differs from the old
	 * value. Direct calling this method with parameter dirty equal to <code>false</code> when there are unsaved changes in viewers, results in
	 ...
	 * 
	 * @param source
	 *            the object that fired <code>PropertyChangeEvent</code> modifying the dirty state
	 * @param dirty
	 ...
	 * 
	 * @param pm
	 *            an <code>IProgressMonitor</code> that the implementation of save may use to show progress
	 ...
	 * were prepared in the foreground (i.e the UI was blocked when the {@link #run(IProgressMonitor)} method (and indirectly the {@link #prepareInput(IProgressMonitor)} method) was invoked. Subclasses
	 * may override.
	 * 
	...
	/**
	 * Return the label to be used for the <code>OK</code> button when this input is displayed in a dialog.
	...
	private boolean saveChanges() {
		try {
			...
			ErrorDialog.openError(
					fComposite.getShell(),
					CompareMessages.CompareDialog_error_title,
					null,
					new Status(IStatus.ERROR, CompareUIPlugin.PLUGIN_ID, 0, NLS.bind(CompareMessages.CompareDialog_error_message,
							x.getTargetException().getMessage()), x.getTargetException()));
		}
		return false;
	}
	 ...
	 * The result of this method should only be considered if {@link #isEditionSelectionDialog()} returns <code>true</code>.
	 * 
