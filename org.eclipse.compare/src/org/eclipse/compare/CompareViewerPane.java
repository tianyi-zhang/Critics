...
/**
 * A <code>CompareViewerPane</code> is a convenience class which installs a <code>CLabel</code> and a <code>Toolbar</code> in a <code>ViewForm</code>.
 * <P>
 * Double clicking onto the <code>CompareViewerPane</code>'s title bar maximizes the <code>CompareViewerPane</code> to the size of an enclosing <code>Splitter</code> (if there is one). If more <code>Splitters</code> are nested maximizing walks up and maximizes to the outermost <code>Splitter</code>.
 * 
 * @since 2.0
 */
public class CompareViewerPane extends ViewForm implements ISelectionProvider, IDoubleClickListener, ISelectionChangedListener,
		IOpenListener, IAdaptable {
	 ...
	 * 
	 * @param container
	 *            a widget which will be the container of the new instance (cannot be null)
	 * @param style
	...
	/**
	 * Returns a <code>ToolBarManager</code> if the given parent is a <code>CompareViewerPane</code> or <code>null</code> otherwise.
	 * 
	 * @param parent
	 ...
	 * 
	 * @param input
	 *            the new input object or <code>null</code>
	 * @since 3.3
	 */
	...
	public void doubleClick(DoubleClickEvent event) {
		if (event.getSelection() instanceof TreeSelection) {
			TreeSelection treeSelect = (TreeSelection) event.getSelection();
			if (treeSelect.getFirstElement() instanceof DiffNode) {
				DiffNode diffNode = (DiffNode) treeSelect.getFirstElement();
				getDiffInfo(diffNode);
			}
		}
	...
	/**
	 * @date mksong Oct 17, 2013 3:20:52 PM
	 * @deprecated
	 * @param event
	 */
	void procLocalHistoryDiff(DoubleClickEvent event) {
		if (event.getSelection() instanceof IStructuredSelection) {
			IStructuredSelection ss = (IStructuredSelection) event.getSelection();
			ICompareInput input = null;
			if (ss.size() == 1)
				input = (ICompareInput) ss.getFirstElement();
			if (input != null) {
			}
		}
	}
	...
	/**
	 * @date mksong Oct 19, 2013 9:30:09 PM
	 * @param input
	 * @param leftEl
	 * @param rightEl
	 */
	void getDiffInfo(DiffNode input) {
		DocumentRangeNode leftEl = null, rightEl = null;
		if (input.getLeft() instanceof DocumentRangeNode || input.getRight() instanceof DocumentRangeNode) {
			leftEl = (DocumentRangeNode) input.getLeft();
			rightEl = (DocumentRangeNode) input.getRight();
			/* added */
			if (leftEl != null && rightEl == null) {
				System.out.println("DBG__________________________________________");
				System.out.println("[DBG] ADDED:");
				System.out.println("DBG__________________________________________");
				System.out.println("[DBG] " + getDiffInfo(input.getLeft().getName(), leftEl));
				System.out.println("[DBG] " + getStringFromInputStream(leftEl.getContents()));
			}
			/* deleted */
			else if (leftEl == null && rightEl != null) {
				System.out.println("DBG__________________________________________");
				System.out.println("[DBG] DELETED:");
				System.out.println("DBG__________________________________________");
				System.out.println("[DBG] " + getDiffInfo(input.getRight().getName(), rightEl));
				System.out.println("[DBG] " + getStringFromInputStream(rightEl.getContents()));
			}
			/* changed */
			else if (leftEl != null && rightEl != null) {
				System.out.println("DBG__________________________________________");
				System.out.println("[DBG] CHGANGED:");
				System.out.println("DBG__________________________________________");
				System.out.println("[DBG] ! LEFT:");
				System.out.println("[DBG] " + getDiffInfo(input.getLeft().getName(), leftEl));
				System.out.println("[DBG] " + getStringFromInputStream(rightEl.getContents()));

				System.out.println("[DBG] ! RIGHT:");
				System.out.println("[DBG] " + getDiffInfo(input.getRight().getName(), rightEl));
				System.out.println("[DBG] " + getStringFromInputStream(leftEl.getContents()));
			}
		}
	 ...
	 * @param node
	 */
	String getDiffInfo(String info, DocumentRangeNode node) {
		int offset = node.getRange().offset;
		int length = node.getRange().length;
		return "SHORT INFO: " + info + ", OFFSET: " + offset + ", LENGTH: " + length;
	}
