 ...
 * <p>
 * In the three-way compare case ranges of differing lines are highlighted and framed with different colors to show whether the difference is an incoming, outgoing, or conflicting change. The <code>TextMergeViewer</code> supports the notion of a current "differing range" and provides toolbar buttons to navigate from one range to the next (or previous).
 * <p>
 * If there is a current "differing range" and the underlying document is editable the <code>TextMergeViewer</code> enables actions in context menu and toolbar to copy a range from one side to the other side, thereby performing a merge operation.
 * <p>
 * In addition to a line-by-line comparison the <code>TextMergeViewer</code> uses a token based compare on differing lines. The token compare is activated when navigating into a range of differing lines. At first the lines are selected as a block. When navigating into this block the token compare shows for every line the differing token by selecting them.
 * <p>
...
public class TextMergeViewer extends ContentMergeViewer implements IAdaptable {
	...
	private static final String[]			GLOBAL_ACTIONS					= { ActionFactory.UNDO.getId(),
			ActionFactory.REDO.getId(), ActionFactory.CUT.getId(), ActionFactory.COPY.getId(), ActionFactory.PASTE.getId(),
	...
	private static final String[]			TEXT_ACTIONS					= { MergeSourceViewer.UNDO_ID,
			MergeSourceViewer.REDO_ID, MergeSourceViewer.CUT_ID, MergeSourceViewer.COPY_ID, MergeSourceViewer.PASTE_ID,
			MergeSourceViewer.DELETE_ID, MergeSourceViewer.SELECT_ALL_ID, MergeSourceViewer.FIND_ID,
	...
	class ContributorInfo implements IElementStateListener, VerifyListener, IDocumentListener, IEncodingSupport {
		...
		public String getEncoding() {
			if (fDocumentProvider != null && fDocumentKey != null && fDocumentProvider instanceof IStorageDocumentProvider) {
				IStorageDocumentProvider provider = (IStorageDocumentProvider) fDocumentProvider;
		...
		/**
		 * Connect to a shared document if possible. Return <code>null</code> if the connection was not possible.
		 * 
		...
		/**
		 * Return the document key used to obtain a shared document. A <code>null</code> is returned in the following cases:
		 * <ol>
		...
		boolean hasSharedDocument(Object object) {
			return (fElement == object && fDocumentProvider != null && fDocumentProvider.getDocument(getDocumentKey()) != null);
		}
	...
	private class ChangeHighlighter implements ITextPresentationListener {
		...
		private StyleRange getStyleRange(Diff diff, IRegion region) {
			...
			// mksong Oct 21, 2013 2:06:15 PM
			// System.out.println("[DBG] " + start + ", " + length);
	...
	private class FindReplaceTarget implements IFindReplaceTarget {
		...
		public int findAndSelect(int widgetOffset, String findString, boolean searchForward, boolean caseSensitive,
			...
			return fFocusPart.getSourceViewer().getFindReplaceTarget()
					.findAndSelect(widgetOffset, findString, searchForward, caseSensitive, wholeWord);
		}
	 ...
	 * 
	 * @param background
	 *            the background color or <code>null</code> to use the system's default background color
	 * @since 2.0
	 */
	 ...
	 * 
	 * @param foreground
	 *            the foreground color or <code>null</code> to use the system's default foreground color
	 * @since 2.0
	 */
	 ...
	 * text viewers have been created for the content areas. The <code>TextMergeViewer</code> implementation of this method will
	 ...
	 * the partitioner returned from {@link #getDocumentPartitioner()} is registered as the default partitioner for the document.
	 * Subclasses that return a partitioner must also override {@link #getDocumentPartitioning()} if they wish to be able to use shared
	 ...
	 * This method is only called if the input provided by the content provider is a <code>IStreamContentAccessor</code> and an internal document must be obtained. This
	 * document is initialized with the partitioner returned from this method.
	 * <p>
	 * The <code>TextMergeViewer</code> implementation of this method returns <code>null</code>. Subclasses may reimplement to create a partitioner for a specific content type. Subclasses that do return a partitioner should also return a partitioning from {@link #getDocumentPartitioning()} in order to make use of shared documents (e.g. file buffers).
	 * 
	...
	/**
	 * Return the partitioning to which the partitioner returned from {@link #getDocumentPartitioner()} is to be associated. Return <code>null</code> only if partitioning is not needed or if the subclass
	 * overrode {@link #setupDocument(IDocument)} directly.
	 * By default, <code>null</code> is returned which means that shared
	 * documents that return a partitioner from {@link #getDocumentPartitioner()} will not be able to use shared documents.
	 * 
	...
	protected void createControls(Composite composite) {
		...
		fAncestor.getSourceViewer().getTextWidget().getAccessible().addAccessibleListener(new AccessibleAdapter() {
			public void getName(AccessibleEvent e) {
				e.result = NLS.bind(CompareMessages.TextMergeViewer_accessible_ancestor, getCompareConfiguration()
		...
		if (part != null) {
			...
			if (selectionProvider instanceof CompareEditorSelectionProvider) {
				...
				cesp.setViewers(
						new SourceViewer[] { fLeft.getSourceViewer(), fRight.getSourceViewer(), fAncestor.getSourceViewer() },
						focusSourceViewer);
			}
		}
	...
	protected final Control createCenterControl(Composite parent) {
		if (fSynchronizedScrolling) {
			...
			if (fUseResolveUI) {
				...
				fLeftToRightButton.setToolTipText(Utilities.getString(getResourceBundle(), "action.CopyDiffLeftToRight.tooltip")); //$NON-NLS-1$
				...
				fRightToLeftButton.setToolTipText(Utilities.getString(getResourceBundle(), "action.CopyDiffRightToLeft.tooltip")); //$NON-NLS-1$
	 ...
	 * initializing the content areas of the merge viewer (see {@link #createControls(Composite)}). It is called three
	...
	/**
	 * Returns an editor input for the given source viewer. The method returns <code>null</code> when no input is available, for example when the input
	...
	private MergeSourceViewer createPart(Composite parent) {
		final MergeSourceViewer viewer = new MergeSourceViewer(createSourceViewer(parent, getDirection()), getResourceBundle(),
			...
			public void lineGetBackground(LineBackgroundEvent event) {
				...
				if (textWidget != null) {
					...
					if (event.lineOffset <= caret && caret <= event.lineOffset + length) {
						...
						Diff diff = findDiff(viewer, event.lineOffset, event.lineOffset + length);
						if (diff != null && updateDiffBackground(diff)) {
							// highlights only the event line, not the
							// whole diff
							event.lineBackground = getColor(fComposite.getDisplay(), getFillColor(diff));
						}
					}
				}
			}
	 ...
	 * 
	 * @param newInput
	 *            the new input of this viewer, or <code>null</code> if there is no new input
	 * @param oldInput
	 *            the old input element, or <code>null</code> if there was previously no input
	...
	private void updateDirtyState(IEditorInput key, IDocumentProvider documentProvider, char type) {
		...
		if (type == LEFT_CONTRIBUTOR)
			...
			setRightDirty(dirty);
		if ((oldLeftDirty && !isLeftDirty()) || (oldRightDirty && !isRightDirty())) {
			/*
	 ...
	 * 
	 * @param left
	 *            if <code>true</code> the contents of the left side is returned; otherwise the right side
		...
		if (v != null) {
			...
			if (d != null) {
				...
				if (contents != null) {
					byte[] bytes;
					try {
						bytes = contents.getBytes(left ? fLeftContributor.internalGetEncoding() : fRightContributor
								.internalGetEncoding());
	...
	protected final void handleResizeLeftRight(int x, int y, int width1, int centerWidth, int width2, int height) {
		...
		if (fBirdsEyeCanvas != null) {
			...
			fBirdsEyeCanvas.setBounds(x + scrollbarWidth, y, BIRDS_EYE_VIEW_WIDTH, height
	...
	private void doDiff() {
		...
		try {
			fMerger.doDiff();
			UTCriticsTextSelection.docMerger = fMerger;

			// mksong Oct 21, 2013 2:18:46 PM
			// System.out.println("DBG__________________________________________");
			// findDiff(lDoc, fLeft);
			// System.out.println("DBG__________________________________________");
			// findDiff(lDoc, fRight);
			// System.out.println("DBG__________________________________________");
			/*
			Diff[] changeDiffs = fMerger.getChangeDiffs(getLeg(viewer), region);
			for (int i = 0; i < changeDiffs.length; i++) {
	 ...
	 * @param msViewer
	 */
	void findDiff(IDocument doc, MergeSourceViewer msViewer) {
		IRegion r = new Region(0, doc.getLength());

		Diff[] changeDiffs = fMerger.getChangeDiffs(getLeg(msViewer), r);
		for (int i = 0; i < changeDiffs.length; i++) {
			Diff leftDiff = changeDiffs[i];
			System.out.println(leftDiff.changeType());

			Position leftPos = leftDiff.getPosition(LEFT_CONTRIBUTOR);
			int left_offset_bgn = leftPos.offset;
			int left_offset_end = left_offset_bgn + leftPos.length;
			System.out.println("[DBG] " + left_offset_bgn + ", " + left_offset_end);

			Position rightPos = leftDiff.getPosition(RIGHT_CONTRIBUTOR);
			int right_offset_bgn = rightPos.offset;
			int right_offset_end = right_offset_bgn + rightPos.length;
			System.out.println("[DBG] " + right_offset_bgn + ", " + right_offset_end);
		}
	}
	...
	private boolean isNavigationButtonEnabled(boolean down, boolean deep) {
		String value = fPreferenceStore.getString(ICompareUIConstants.PREF_NAVIGATION_END_ACTION);
		...
		if (v != null) {
			...
			if (document != null) {
				...
				try {
					...
					// mksong Oct 20, 2013 11:41:45 PM
					// System.out.println("[DBG] lineoffset: " + lineOffset + ", offset: " + offset);

					String format = CompareMessages.TextMergeViewer_cursorPosition_format;
					return MessageFormat
							.format(format, new String[] { Integer.toString(line + 1), Integer.toString(column + 1) });
	...
	protected void createToolItems(ToolBarManager tbm) {
		...
		fHandlerService = CompareHandlerService.createFor(getCompareConfiguration().getContainer(), fLeft.getSourceViewer()
				.getControl().getShell());
		...
		showWhitespaceAction = new ShowWhitespaceAction(new MergeSourceViewer[] { fLeft, fRight, fAncestor }, new boolean[] {
				needsLeftPainter, needsRightPainter, needsAncestorPainter });
		...
		toggleLineNumbersAction = new TextEditorPropertyAction(CompareMessages.TextMergeViewer_16, new MergeSourceViewer[] {
				fLeft, fRight, fAncestor }, AbstractDecoratedTextEditorPreferenceConstants.EDITOR_LINE_NUMBER_RULER);
	...
	private void configureCompareFilterActions(Object input, Object ancestor, Object left, Object right) {
		if (getCompareConfiguration() != null) {
			...
			Object current = getCompareConfiguration().getProperty(ChangeCompareFilterPropertyAction.COMPARE_FILTER_ACTIONS);
			...
			if (current != null && current instanceof List && ((List) current).size() == compareFilterDescriptors.length) {
				...
				for (int i = 0; i < compareFilterDescriptors.length; i++) {
					...
					for (int j = 0; j < currentFilterActions.size(); j++) {
						if (compareFilterDescriptors[i].getFilterId().equals(
								((ChangeCompareFilterPropertyAction) currentFilterActions.get(j)).getFilterId())) {
							match = true;
							break;
						}
					}
			...
			if (!currentFiltersMatch) {
					...
					if (getCompareConfiguration().getContainer().getActionBars() != null) {
						...
						if (compareFilterAction.getActionDefinitionId() != null)
							getCompareConfiguration().getContainer().getActionBars()
									.setGlobalActionHandler(compareFilterAction.getActionDefinitionId(), compareFilterAction);
					}
				}
				...
				for (int i = 0; i < fCompareFilterActions.size(); i++) {
					((ChangeCompareFilterPropertyAction) fCompareFilterActions.get(i)).setInput(input, ancestor, left, right);
				}
			}
		}
	}
	...
	private void disposeCompareFilterActions(boolean updateActionBars) {
		Iterator compareFilterActionsIterator = fCompareFilterActions.iterator();
		while (compareFilterActionsIterator.hasNext()) {
			...
			if (updateActionBars && getCompareConfiguration().getContainer().getActionBars() != null) {
				getCompareConfiguration().getContainer().getActionBars().getToolBarManager().remove(compareFilterAction.getId());
				if (compareFilterAction.getActionDefinitionId() != null)
					getCompareConfiguration().getContainer().getActionBars()
							.setGlobalActionHandler(compareFilterAction.getActionDefinitionId(), null);
			}
			compareFilterAction.dispose();
		}
		if (updateActionBars && !fCompareFilterActions.isEmpty()
	...
	protected void handlePropertyChangeEvent(PropertyChangeEvent event) {
		...
		if (key.equals(CompareConfiguration.IGNORE_WHITESPACE)
				...
				|| (key.equals(ChangeCompareFilterPropertyAction.COMPARE_FILTERS) && getCompareConfiguration().getProperty(
	...
	static RGB interpolate(RGB fg, RGB bg, double scale) {
		if (fg != null && bg != null)
			return new RGB((int) ((1.0 - scale) * fg.red + scale * bg.red), (int) ((1.0 - scale) * fg.green + scale * bg.green),
	...
	private void handleEndOfDocumentReached(Shell shell, boolean next) {
		...
		if (!value.equals(ICompareUIConstants.PREF_VALUE_PROMPT)) {
			...
			NavigationEndDialog dialog = new NavigationEndDialog(shell, title, null, message, new String[][] { localLoopOption,
	...
	void displyDiffInfo(Diff diff) {
		// mksong Oct 20, 2013 4:01:10 PM
		...
		if (diff != null && UTCriticsEnable.autoSelection) {
			...
			// diff.getChangeDiffs(LEFT_CONTRIBUTOR, region)

			/*
			Position leftDiffPosition = diff.getPosition(LEFT_CONTRIBUTOR);
			Position rightDiffPosition = diff.getPosition(RIGHT_CONTRIBUTOR);

			int left_offset_bgn = leftDiffPosition.getOffset();
			int left_offset_end = left_offset_bgn + rightDiffPosition.getLength();

			int right_offset_bgn = diff.getPosition(RIGHT_CONTRIBUTOR).getOffset();
			int right_offset_end = right_offset_bgn + diff.getPosition(RIGHT_CONTRIBUTOR).getLength();

			int left_bgn_line, right_bgn_line, left_end_line, right_end_line;
			try {
				IDocument leftDoc = fLeft.getSourceViewer().getDocument();
				left_bgn_line = leftDoc.getLineOfOffset(left_offset_bgn);
				left_end_line = leftDoc.getLineOfOffset(left_offset_end);

				IDocument rightDoc = fRight.getSourceViewer().getDocument();
				right_bgn_line = rightDoc.getLineOfOffset(right_offset_bgn);
				...
				System.out.println(leftDoc.get().substring(left_offset_bgn, left_offset_end));
				System.out.println("DBG__________________________________________");
				...
				System.out.println(rightDoc.get().substring(right_offset_bgn, right_offset_end));
				System.out.println("DBG__________________________________________");
			} catch (BadLocationException e) {
				e.printStackTrace();
			}
			*/
			// UTCriticsTextSelection.leftSelectedDiff = getDiffRange(fLeft, diff.getPosition(LEFT_CONTRIBUTOR));
			// UTCriticsTextSelection.rightSelectedDiff = getDiffRange(fRight, diff.getPosition(RIGHT_CONTRIBUTOR));
		}
	}
	...
	void copyAllUnresolved(boolean leftToRight) {
		if (fMerger.hasChanges() && isThreeWay() && !isIgnoreAncestor()) {
			IRewriteTarget target = leftToRight ? fRight.getSourceViewer().getRewriteTarget() : fLeft.getSourceViewer()
					.getRewriteTarget();
	...
	public Object getAdapter(Class adapter) {
		...
		if (adapter == CompareHandlerService[].class) {
			return new CompareHandlerService[] { fHandlerService, super.getCompareHandlerService() };
		}
	...
	/**
	 * This method returns {@link ITextEditor} used in the {@link ChangeEncodingAction}. It provides implementation of methods that
