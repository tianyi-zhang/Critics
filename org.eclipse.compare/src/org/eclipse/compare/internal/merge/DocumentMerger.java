...
public class DocumentMerger {
	...
	public class Diff {
		 ...
		 */
		Diff(Diff parent, int dir, IDocument ancestorDoc, Position aRange, int ancestorStart, int ancestorEnd, IDocument leftDoc,
				Position lRange, int leftStart, int leftEnd, IDocument rightDoc, Position rRange, int rightStart, int rightEnd) {
		...
		public int getMaxDiffHeight() {
			...
			if (isThreeWay())
				h = Math.max(h,
						getLineRange(getDocument(MergeViewerContentProvider.ANCESTOR_CONTRIBUTOR), fAncestorPos, region).y);
		...
		public void setKind(int direction) {
	...
	public void doDiff() throws CoreException {
		...
		try {
			...
			Diff diff = new Diff(null, RangeDifference.NOCHANGE, aDoc, aRegion, 0, aDoc != null ? aDoc.getLength() : 0, lDoc,
					lRegion, 0, lDoc.getLength(), rDoc, rRegion, 0, rDoc.getLength());

			fAllDiffs = new ArrayList();
			fAllDiffs.add(diff);
			throw new CoreException(new Status(IStatus.ERROR, CompareUIPlugin.PLUGIN_ID, 0, CompareMessages.DocumentMerger_1,
		...
		if (isCapped(sa, sl, sr))
			fInput.getCompareConfiguration().setProperty(CompareContentViewerSwitchingPane.OPTIMIZED_ALGORITHM_USED,
					new Boolean(true));
		else
			fInput.getCompareConfiguration().setProperty(CompareContentViewerSwitchingPane.OPTIMIZED_ALGORITHM_USED,
					new Boolean(false));
		...
		for (int i = 0; i < e.length; i++) {
			...
			Diff diff = new Diff(null, es.kind(), aDoc, aRegion, ancestorStart, ancestorEnd, lDoc, lRegion, leftStart, leftEnd,
	...
	private boolean isCapped(DocLineComparator ancestor, DocLineComparator left, DocLineComparator right) {
		...
		if ((double) aLength * (double) lLength > LCS.TOO_LONG || (double) aLength * (double) rLength > LCS.TOO_LONG
	...
	public Diff findDiff(char type, int pos) throws CoreException {
		...
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, CompareUIPlugin.PLUGIN_ID, 0, CompareMessages.DocumentMerger_3,
					ex.getTargetException()));
		} catch (InterruptedException ex) {
			//
		}
		...
		if (e != null) {
			for (int i = 0; i < e.length; i++) {
				...
				Diff diff = new Diff(null, kind, aDoc, aRegion, ancestorStart, ancestorEnd, lDoc, lRegion, leftStart, leftEnd,
						rDoc, rRegion, rightStart, rightEnd);
	...
	private void mergingTokenDiff(Diff baseDiff, IDocument ancestorDoc, String a, IDocument rightDoc, String d,
		...
		for (int i = 0; i < r.length; i++) {
			...
			if (first != null && last != null) {
				...
				Diff diff = new Diff(baseDiff, first.kind(), ancestorDoc, null, ancestorStart2, ancestorEnd2, leftDoc, null,
						leftStart2, leftEnd2, rightDoc, null, rightStart2, rightEnd2);
				diff.fIsToken = true;
				baseDiff.add(diff);
			}
		}
	...
	private void simpleTokenDiff(final Diff baseDiff, IDocument ancestorDoc, String a, IDocument rightDoc, String d,
		...
		for (int i = 0; i < e.length; i++) {
			...
			if (kind != RangeDifference.NOCHANGE) {
				...
				Diff diff = new Diff(baseDiff, kind, ancestorDoc, null, ancestorStart2, ancestorEnd2, leftDoc, null, leftStart2,
						leftEnd2, rightDoc, null, rightStart2, rightEnd2);
	...
	public Diff findDiff(int viewportHeight, boolean synchronizedScrolling, Point size, int my) {
		...
		if (fAllDiffs != null) {
			...
			while (e.hasNext()) {
				Diff diff = (Diff) e.next();
				int h = synchronizedScrolling ? diff.getMaxDiffHeight() : diff.getRightHeight();
