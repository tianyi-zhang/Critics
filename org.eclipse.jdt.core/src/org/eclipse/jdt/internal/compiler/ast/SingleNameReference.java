...
public class SingleNameReference extends NameReference implements OperatorIds {
	...
	public FlowInfo analyseAssignment(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo, Assignment assignment, boolean isCompound) {
		...
		case Binding.FIELD: // assigning to a field
			...
			} else if (!isCompound && fieldBinding.isNonNull() && fieldBinding.declaringClass == currentScope.enclosingReceiverType()) { // inherited fields are not tracked here
	...
	public TypeBinding checkFieldAccess(BlockScope scope) {
		...
		if (fieldBinding.isStatic()) {
			...
			if (declaringClass.isEnum()) {
				...
				if (this.constant == Constant.NotAConstant && !methodScope.isStatic && (sourceType == declaringClass || sourceType.superclass == declaringClass) // enum constant body
		...
		if ((this.bits & ASTNode.IsStrictlyAssigned) == 0 && methodScope.enclosingSourceType() == fieldBinding.original().declaringClass && methodScope.lastVisibleFieldID >= 0 && fieldBinding.id >= methodScope.lastVisibleFieldID && (!fieldBinding.isStatic() || methodScope.isStatic)) {
	...
	public void generateAssignment(BlockScope currentScope, CodeStream codeStream, Assignment assignment, boolean valueRequired) {
		...
		if (assignment.expression.isCompactableOperation()) {
			...
			if ((operation.right instanceof SingleNameReference) && ((operator == OperatorIds.PLUS) || (operator == OperatorIds.MULTIPLY)) // only commutative operations
					&& ((variableReference = (SingleNameReference) operation.right).binding == this.binding) && (operation.left.constant != Constant.NotAConstant) // exclude non constant expressions, since could have side-effect
		...
		case Binding.LOCAL: // assigning to a local variable
			...
			if (localBinding.type.isArrayType() && ((assignment.expression instanceof CastExpression) // arrayLoc = (type[])null
			...
			case Binding.FIELD: // reading a field
				...
				if (codegenField.isStatic()) {
					...
					// if no valueRequired, still need possible side-effects of <clinit> invocation, if field belongs to different class
							&& ((FieldBinding) this.binding).original().declaringClass == this.actualReceiverType.erasure() && ((this.implicitConversion & TypeIds.UNBOXING) == 0) && this.genericCast == null) {
	...
	public void manageSyntheticAccessIfNecessary(BlockScope currentScope, FlowInfo flowInfo, boolean isReadAccess) {
		...
		if ((this.bits & Binding.FIELD) != 0) {
			...
			if (((this.bits & ASTNode.DepthMASK) != 0) && (codegenField.isPrivate() // private access
				...
				this.syntheticAccessors[isReadAccess ? SingleNameReference.READ : SingleNameReference.WRITE] = ((SourceTypeBinding) currentScope.enclosingSourceType().enclosingTypeAt((this.bits & ASTNode.DepthMASK) >> ASTNode.DepthSHIFT)).addSyntheticMethod(codegenField, isReadAccess, false /*not super access*/);
	...
	public StringBuffer printExpression(int indent, StringBuffer output) {
		if (JavaNewNamePrint.printWithNewName && newName != null) {
			return output.append(newName);
		} else {
			...
			case Binding.VARIABLE | Binding.TYPE: // ====both variable and type============
				if (this.binding instanceof VariableBinding) {
					...
					if (variableType != null) {
						this.resolvedType = variableType = (((this.bits & ASTNode.IsStrictlyAssigned) == 0) ? variableType.capture(scope, this.sourceEnd) : variableType);
