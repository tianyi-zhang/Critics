...
public class CompilationUnitEditor extends JavaEditor implements IJavaReconcilingListener {
	...
	private class BracketInserter implements VerifyKeyListener, ILinkedModeListener {
		...
		public void verifyKey(VerifyEvent event) {
			...
			try {
				...
				String previous = prevToken == Symbols.TokenEOF ? null : document.get(prevTokenOffset, offset - prevTokenOffset)
						.trim();

				switch (event.character) {
				case '(':
					if (!fCloseBrackets || nextToken == Symbols.TokenLPAREN || nextToken == Symbols.TokenIDENT || next != null
							&& next.length() > 1)
						return;
					break;
				...
				case '<':
					if (!(fCloseAngularBrackets && fCloseBrackets) || nextToken == Symbols.TokenLESSTHAN
							...
							&& isTypeArgumentStart(next) || prevToken != Symbols.TokenLBRACE && prevToken != Symbols.TokenRBRACE
				...
				case '\'':
				case '"':
					if (!fCloseStrings || nextToken == Symbols.TokenIDENT || prevToken == Symbols.TokenIDENT || next != null
							&& next.length() > 1 || previous != null && previous.length() > 1)
						return;
					break;
				...
				level.fUI
						.setExitPolicy(new ExitPolicy(closingCharacter, getEscapeCharacter(closingCharacter), fBracketLevelStack));
	...
	protected void createActions() {
		...
		if (action != null) {
			...
			action.setToolTipText(JavaEditorMessages.getBundleForConstructedKeys().getString(
					"ContentAssistContextInformation.tooltip")); //$NON-NLS-1$
			action.setDescription(JavaEditorMessages.getBundleForConstructedKeys().getString(
					"ContentAssistContextInformation.description")); //$NON-NLS-1$
		}

		action = new TextOperationAction(JavaEditorMessages.getBundleForConstructedKeys(),
		...
		JavaMoveLinesAction[] moveLinesActions = JavaMoveLinesAction.createMoveCopyActionSet(
				JavaEditorMessages.getBundleForConstructedKeys(), this);
	...
	/**
	 * Returns the most narrow element including the given offset. If <code>reconcile</code> is <code>true</code> the editor's input element is reconciled in advance. If it is <code>false</code> this method only returns a result if the editor's input element
	...
	public void doSave(IProgressMonitor progressMonitor) {
		...
		if (p.isDeleted(getEditorInput())) {
			...
			if (isSaveAsAllowed()) {
				...
				MessageDialog.openError(shell, JavaEditorMessages.CompilationUnitEditor_error_saving_title1,
						JavaEditorMessages.CompilationUnitEditor_error_saving_message1);
			}
			...
			// mksong Oct 17, 2013 12:42:19 PM
			// System.out.println("------------------------------------------");
			// System.out.println("[DBG] SELECTED CODE BLOCK");
			// System.out.println("------------------------------------------");
			// try {
			// Point offset = getViewer().getSelectedRange();
			// if (offset.y == 0) {
			...
			protected Control createMessageArea(Composite parent) {
				...
				link.addSelectionListener(new SelectionAdapter() {
					...
					public void widgetSelected(SelectionEvent e) {
						if (hasProjectSettings)
							PreferencesUtil.createPropertyDialogOn(getShell(), javaProject,
									SaveParticipantPreferencePage.PROPERTY_PAGE_ID, null, null).open();
						else
